...
let modalDelay = 1000;
let url = 'https://cs3660-nelsonj3.c9users.io/api/v1/students';
...
function loadStudentIds() {
    $.when($.ajax(url + '/students.json')).done(function(data, statusText, jqXHR) {
        studentIds = data;
    });
}
...
function loadPage(page) {
    ...
    for (var i = 0; i < count; i++) {
        $.when($.ajax(url + '/' + studentIds[i + offset] + '.json', {context: {url: url + '/' + studentIds[i + offset] + '.json'}})).done(function(data, statusText, jqXHR) {
            studentCount++;
